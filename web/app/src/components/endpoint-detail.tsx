            ...
            <div className="space-y-6">
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <div key={card.label} className="rounded-lg border border-border bg-card">
                    <div className="px-6 pb-2 pt-6"><p className="text-sm font-medium text-muted-foreground">{card.label}</p></div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    {['30d', '7d', '24h', '1h'].map((period) => (
                      <div key={period} className="rounded-lg border border-border bg-card">
                        <div className="px-6 pb-2 pt-6 text-center"><p className="text-sm font-medium text-muted-foreground">{period === '30d' ? 'Last 30 days' : period === '7d' ? 'Last 7 days' : period === '24h' ? 'Last 24 hours' : 'Last hour'}</p></div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    {['30d', '7d', '24h', '1h'].map((period) => (
                      <div key={period} className="text-center">
                        <p className="mb-2 text-sm text-muted-foreground">{period === '30d' ? 'Last 30 days' : period === '7d' ? 'Last 7 days' : period === '24h' ? 'Last 24 hours' : 'Last hour'}</p>
                  ...
                  <div className="p-6 space-y-4">
                    {events.map((ev, i) => (
                      <div key={i} className="flex items-start gap-4 border-b border-border pb-4 last:border-0">
                        <div className="mt-1">
                          {ev.type === 'HEALTHY' ? <ArrowUpCircle className="h-5 w-5 text-green-500" /> : ev.type === 'UNHEALTHY' ? <ArrowDownCircle className="h-5 w-5 text-red-500" /> : <PlayCircle className="h-5 w-5 text-muted-foreground" />}
                        </div>
                        <div className="flex-1">
                          <p className="font-medium">{ev.fancyText}</p>
                          <p className="text-sm text-muted-foreground">{formatTimestamp(ev.timestamp)} {'\u2022'} {ev.fancyTimeAgo}</p>
                        </div>
                      </div>
                    ))}
                  </div>
